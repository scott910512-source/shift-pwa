...
**글자가 칸을 넘치거나 겹쳐 보임**
글꼴마다 같은 숫자를 줘도 실제로 그려지는 크기가 다릅니다.
윈도우의 맑은 고딕은 나눔고딕보다 크게 그려집니다.
지금은 글꼴의 실제 높이를 재서 보정하고, 이름 줄은 글자 수가 아니라
실제 폭을 재서 나누므로 어떤 글꼴에서도 넘치지 않습니다.
그래도 이상하면 `--font` 로 다른 글꼴을 지정해 보세요.
