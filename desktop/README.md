...
```
┌─[아이콘 자리]─┬────────────────────────────────────┬──────────────────┐
│               │ Good Work / Better Tomorrow        │ 현재 근무자      │
│               │ 4조 2교대 근무표                    │ ┌──────────────┐ │
│               │ 2026. 09 │ 16일 (수)   ● 주간 ● 야간 │ │ B조 주간     │ │
│               │ ┌────────────────────────────────┐ │ │ 교대조장 …   │ │
│               │ │ SUN MON TUE WED THU FRI SAT    │ │ │ 1·2·3공장 …  │ │
│               │ │  ①   ②   ③   ④   ⑤   ⑥   ⑦   │ │ └──────────────┘ │
│               │ │ 주C 주C 주B 주B 주A 주A 주A     │ │ ┌──────────────┐ │
│               │ │ 야A 야A 야D 야D 야C 야C 야C     │ │ │ D조 야간     │ │
│               │ │  …  (오늘은 파랗게)             │ │ └──────────────┘ │
│               │ └────────────────────────────────┘ │ ┌ 상근 ─────────┐ │
│               │ SAFETY · PEOPLE · TECHNOLOGY …     │ │ 방우석 …      │ │
└───────────────┴────────────────────────────────────┴──────────────────┘
        배경: 세종호수공원 · 이응다리 (직접 그림) 또는 사진
```

- **왼쪽** — 바탕화면 아이콘 자리로 비워 둡니다 (기본 5칸).
- **가운데** — 이번 달 달력. 날짜마다 `주 ○` / `야 ○` 로 어느 조인지 표시하고,
  오늘은 파란 칸입니다. 주는 일요일부터 시작합니다.
- **오른쪽** — 지금 근무 중인 주간·야간 조와 공장별 명단, 그리고 상근.

## 배경 사진 바꾸기

기본은 세종호수공원과 이응다리를 단순하게 **그린 그림**입니다.
사진을 쓰고 싶으면 `wallpaper.py` 옆에 **`background.jpg`** 를 두면 그걸 씁니다.
(화면 비율에 맞춰 잘라서 채웁니다.) 실행할 때 `--bg 사진.jpg` 로 줘도 됩니다.

## 글귀 바꾸기

`wallpaper.py` 위쪽에서 고칠 수 있습니다. 빈 문자열로 두면 그 줄이 사라집니다.
